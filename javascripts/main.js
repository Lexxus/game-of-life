...
setTimeout(function(){
	var zoom = 6,
		...
		lifeBody = document.getElementById('main_content_wrap'),
		w = lifeBody.clientWidth,
		h = lifeBody.clientHeight,
	...
	*/
	...
	var canvasOffset = {left: gd.canvas.offsetLeft, top: gd.canvas.offsetTop},
		el = gd.canvas;
	while(el.offsetParent){
		el = el.offsetParent;
		canvasOffset.left += el.offsetLeft;
		canvasOffset.top += el.offsetTop;
	}
	
	gd.canvas.onmousedown = function(e){
		console.log('offset: '+canvasOffset.left+','+canvasOffset.top);
		if(allowDrawing){
			var xy = gd.convertXY(e.pageX - canvasOffset.left, e.pageY - canvasOffset.top);
